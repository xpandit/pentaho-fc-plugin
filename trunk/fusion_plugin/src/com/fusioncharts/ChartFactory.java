...
public class ChartFactory extends Object {
	...
	{				
		...
		//We need to get the graph, and declare the holder for the series in the graph
	...
	throws WritingException
	{
		...
		for (int i=0; i < graph.getNumberOfCategories(); i++) {
			...
			xmlwriter.writeAttribute("label", escapeGoofyCharacters(graph.getCategory(i)));
			xmlwriter.endEntity();
		}
		xmlwriter.endEntity();
	...
	{		
		...
		String swfPath=PentahoSystem.getApplicationContext().getFullyQualifiedServerURL()+"/content/fusion/charts/";

		String domWrapper =
			"<fusioncharts chartType=\""+chartType+"\" width=\""+width+"\" height=\""+height+"\" SWFPath=\""+swfPath+"\">" +
			"<data><!--[CDATA[" +
			dataXml +
			"]]--></data>" +
			"</fusioncharts>";
